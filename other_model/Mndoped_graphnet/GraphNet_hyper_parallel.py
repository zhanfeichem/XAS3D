...
from torch_geometric.nn import GraphConv,GATConv,GATv2Conv
...
class GraphNet(torch.nn.Module):
    def __init__(self, out_channels=239, hidden_channels=32, in_channels=4,n_layers=3,dropout=0.5,jk='lstm'):
        super().__init__()
        self.dropout=dropout
        
        self.layers_in=GraphConv(in_channels, hidden_channels,jk=jk)
        self.layers_out=GraphConv(hidden_channels, out_channels, concat=False, jk=jk)
        layers=[]
        for i in range(n_layers-2):
            tmp=GraphConv(hidden_channels , hidden_channels,jk=jk)
            layers.append(tmp)
...
########################################################################Setting
setting_prefix="Mn03_EF3def_ihs6000_site"
setting_n_spc=239#239 120
...
# params = dict({'hidden_channels': 64, 'num_layers': 4, 'num_heads': 4, 'dropout': 0.8, 'lr': 0.0005})
    ...
    model = GraphNet(n_layers=num_layers,in_channels=4,out_channels=setting_n_spc,hidden_channels=hidden_channels,jk='lstm').to(device)
