...
from input_Fe import *
epochs=setting_n_ep
n_ep_save=setting_n_ep_save
cutoff=setting_cutoff
npt=setting_npt
#3 228 202 6 12 2
model = XAS3Dabs(out_channels=npt,cutoff=cutoff,num_layers=3,hidden_channels=228,middle_channels=202,num_output_layers=6,num_radial=12,num_spherical=2)
...
dataset_test = dataset_test("valid")
...
for epoch in range(1, epochs + 1):
    ...
    if valid_mae < best_mae:
            ...
            with open("1best.log","w") as fbest:
                fbest.write("BEST MAE: "+str(best_mae)+" EPOCH "+str(epoch)+"\n")
...
weight_obj=2*(exp[:,0]>setting_weight_xval)+1*(exp[:,0]<setting_weight_xval)
...
def opt_sub_conv():
    ...
    x0=np.array([norm*1.0,0,7113,17,21,39])
...
npar=168
opt = nlopt.opt(nlopt.LN_NELDERMEAD, npar)  #GN_DIRECT_L  LN_COBYLA  LN_BOBYQA  LN_NELDERMEAD  GN_ESCH GN_ISRES  GN_CRS2_LM   GN_MLSL  GN_AGS
...
def obj_structure_plt_unconv(x,grad=None):
    ...
    plt.title("OBJ VAL:" + "%.2f NORM %.2f SHIFT %.2f (eV) Fermi %.2f (eV) Gmax %.2f Ecent %.2f Elarge %.2f" % (res, x_conv[0], x_conv[1],x_conv[2],x_conv[3],x_conv[4],x_conv[5])
              ,fontsize=6)
    ##########save opt res
